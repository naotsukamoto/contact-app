...
export const Home2: React.FC = memo(() => {
  const navigate = useNavigate();

  // use-react-queryでusers情報を取得
  const ref = query(
    collection(db, "users"),
    where("uid", "==", "QPb7HajAY4PqQ0ZKTOsW1s9Aj7E2")
  );
  const usersQuery = useFirestoreQuery(["users"], ref);
  const { data: snapshot, isLoading } = usersQuery;
  ...
  return (
    <SContainer>
      {isLoading && (
        <Skeleton style={{ margin: "0 auto" }} height={40} width={200} />
      )}
      {snapshot &&
        snapshot.docs.map((doc) => <p key={doc.id}>{doc.data().user_name}</p>)}
      <ExchangeDay
